...
def run(ceph_cluster, **kw):
    try:
        log.info(f"Running CephFS tests for ceph kernel xfstests")
        ...
        client1.exec_command(
            ...
            check_ec=False,long_running=True
        )
        ...
        #     "umount", kernel_clients=[clients[0]], mounting_dir=test_mount_point,check_ec=False
        # )
        # fs_util.client_clean_up(
        #     "umount", kernel_clients=[clients[0]], mounting_dir=scratch_mount_point,check_ec=False
        # )
        uninstall_commands = [
